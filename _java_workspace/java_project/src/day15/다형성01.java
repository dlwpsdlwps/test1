 ...
 * - 멥버변수(필드)는 숨기고, 메서드로 접근
 * - 멤버변수와 메서드를 하나로 묶어서 처리
 * - 은닉화 : 객체의 내부의 정보는 숨겨 외부로 드러나지 않게 하는 것
 * 			외부에서 데이터에 직접 접근하는 것을 방지
 * 
 * - 상속(Inheritance) - 코드 재사용(확장)
 * - 클래스를 상속받아 수정하며 사용하면 중복 코드를 줄일 수 있음
 * - 유지보수가 편함
 * 
 * - 다형성(Polymorphism) - 객체 형변환 용어
 * - 하나의 코드에 여러 자료(객체)형으로 구현되어 실행되는 것
