...
public class Collection01 {
	...
	public static void main(String[] args) {
		 ...
		 * - 아이디/패스워드와 같이 하나의 자료를 묶어서 저장해야 할 때 사용하는 구조
		 * 
		 * 배열은 기본자료형, 클래스 자료형으로 구성이 가능함
		 * int arr[], String arr[], Student arr[] 다 가능
		 * 
		 * Collection에서는 기본자료형은 불가능함 클래스 자료형만 구성 가능
		 * 기본자료형에 맞는 클래스 자료형을 사용하면 됨
		 * int => Integer, String
		 * 나머지는 기본자료형에 첫 글자만 대문자로 변환하면 클래스가 됨
		...
		//문자열 5개를 저장한 후 출력
		//총 개수가 몇개인지를 출력
		System.out.println("---문자열을 담은 ArrayList---");
		...
		System.out.println("--list1에 1부터 10까지 값을 지정한 후 출력--");
		for(int i=0;i<10;i++) {
			list1.add(i+1);
		}
		...
		//contains(object) : list에 값이 있는지 검사 t/f로 리턴
		...
		//isEmpty() : 리시트 비어있는지 체크 t/f
		System.out.println(list1.isEmpty());
		
		//ArrayList로 number를 생성 숫자를 저장하는 리스트
		//1부터 10까지 채운 후 출력 (향상된 for문을 이용하여 출력)
		System.out.println("ArrayList로 number를 생성 숫자를 저장하는 리스트");
		...
		//indexOf(값) : 해당 값이 list에 있는지 확인 후 있으면 index / 없다면 -1 리턴
		...
		//Comparator 인터페이스를 구현한 구현체를 넣어야 함 (익명클랙스 사용)
		//compareTo 메서드를 구현하여 객체를 정렬
		// - compareTo : 사전 상 앞에 있으면 -1, 같으면 0, 뒤면 1
